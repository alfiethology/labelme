...
- Added Tools > Change Labels as an in-place review mode on the main Labelme canvas. Its shortcut file accepts case-insensitive letter-and-number sequences of any length; typing a sequence and confirming it with Enter or Space renames the selected shape and advances to the next shape, automatically saving and opening the next image after its final shape. Backspace corrects the sequence, shapes remain movable with normal editing controls, and Shift+Space moves a skipped image and annotation into a `skipped_images` directory alongside the current labels directory so skipped files are outside the active session tree.
- Added Tools > Delete Unlabelled Images with a Yes/No confirmation before permanently deleting every image in the open directory that has no corresponding annotation file; the current image is protected when it has unsaved changes.
...
- Fixed Shift+Space in Change Labels mode removing the skipped image from the file list without loading its successor; the replacement row is now selected silently and loaded explicitly, including when removing a row causes the next item to inherit the same index.
- Fixed YOLO pose export treating unrelated JSON files such as `summary.json` as Labelme annotations and crashing on a missing `imagePath`; unrelated JSON is now ignored, while malformed annotation errors are shown in-app instead of closing Labelme. Mixed files remain supported: only Skeleton Shapes are exported and OBB/other shapes are ignored.
...
- Fixed the app crashing when saving a label file failed on a raw filesystem error rather than a wrapped `LabelFileError`; `save_labels` created the output directory (`Path.mkdir`) and computed the image's relative path (`os.path.relpath`) inside its `try` but caught only `LabelFileError`, so a read-only or full `--output` directory (`PermissionError`/`OSError`) or, on Windows, an image and output on different drives (`ValueError`) escaped uncaught and took the whole app down through the global exception hook, including during auto-save. These are now caught like the existing write failures, surfacing the save-error dialog and returning `False` ([#2393](https://github.com/wkentaro/labelme/pull/2393))
