...
- Added Tools > Change Labels as an in-place review mode on the main Labelme canvas. Configured label keys rename the selected shape and advance to the next shape in that image; Enter keeps the current label and advances; shapes remain movable with normal editing controls; Space saves and opens the next image, and Shift+Space moves a skipped image and annotation into `skipped_frames`.
