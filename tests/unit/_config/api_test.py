...
def test_default_label_color_is_yellow() -> None:
    ...
    assert config["shape_color"]["mode"] == "uniform"
    assert config["shape_color"]["uniform"]["color"] == [255, 255, 0]
    assert config["shape_color"]["by_label"]["fallback"] == [255, 255, 0]
    ...
    [
        (
            {"shape_color": "auto", "shift_auto_shape_color": -2},
            {
                "mode": "auto",
                "auto": {"shift": -2},
                "uniform": {"color": [255, 255, 0]},
                "by_label": {"colors": None, "fallback": [255, 255, 0]},
            },
        ),
        (
            {
