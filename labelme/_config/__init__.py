...
def _migrate_config_from_file(config_from_yaml: dict) -> None:
    ...
    if shortcuts.pop("create_rotation", None):
        logger.info(
            "Migrating old config: shortcuts.create_rotation "
    ...
    if isinstance(crosshair, dict):
        if "rotation" in crosshair:
            logger.info(
                "Migrating old config: canvas.crosshair.rotation "
                "-> canvas.crosshair.oriented_rectangle"
            )
            crosshair.setdefault("oriented_rectangle", crosshair.pop("rotation"))
