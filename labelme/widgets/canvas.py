...
class Canvas(QtWidgets.QWidget):
    ...
    zoom_rect_selected = QtCore.pyqtSignal(QRectF)
    ...
    def _dispatch_pointer_move(self, pos: QPointF, event: QtGui.QMouseEvent) -> None:
        if self.zoom_mode and self._zoom_rect_start is not None:
            self._track_zoom_rect_drag(event=event)
            return
    ...
    def _track_zoom_rect_drag(self, event: QtGui.QMouseEvent) -> None:
        self._zoom_rect_end = event.localPos()
        self.update()
    ...
    def _begin_zoom_rect(self, event: QtGui.QMouseEvent) -> None:
        self._zoom_rect_start = event.localPos()
        self._zoom_rect_end = event.localPos()
        self._apply_cursor(CURSOR_DRAW)
        self.update()
    ...
    def _draw_zoom_rect_overlay(self) -> None:
        if (
            not self.zoom_mode
            or self._zoom_rect_start is None
        ...
        try:
            ...
            painter.setPen(QtGui.QPen(QtGui.QColor(0, 120, 215, 200), 2, Qt.DashLine))
    ...
    def keyPressEvent(self, a0: QtGui.QKeyEvent) -> None:
        ...
        key = a0.key()
        if key == Qt.Key_Z:
            self.set_zoom_rect_mode(not self.zoom_mode)
            a0.accept()
            return
        if self.drawing():
