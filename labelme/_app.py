...
""",
        ...
        fill_editing = action(
            self.tr("Fill Editing Shapes"),
            self._canvas_widgets.canvas.set_fill_editing,
            None,
            icon="phosphor/paint-bucket.svg",
            tip=self.tr("Fill selected or hovered shapes while editing"),
            checkable=True,
            enabled=True,
        )
        ...
        show_center_dots = action(
            ...
            None,
            tip=self.tr("Show center dots for rectangles and polygons"),
            checkable=True,
            enabled=True,
        )
        show_center_dots.setChecked(self._config["canvas"]["show_center_dots"])
        self._canvas_widgets.canvas.set_show_center_dots(show_center_dots.isChecked())
        hide_all = action(
            self.tr("&Hide\nShapes"),
        ...
        self._zoom_rect_button = QtWidgets.QPushButton(self.tr("Zoom Rectangle"))
        self._zoom_rect_button.setCheckable(True)
        self._zoom_rect_button.setToolTip(
            self.tr("Select an area to zoom into (shortcut: Z)")
        )
        self._zoom_rect_button.clicked.connect(self.toggle_zoom_rect)
        zoom_box_layout.addWidget(self._zoom_rect_button)
    ...
    def _setup_canvas(self) -> _CanvasWidgets:
        ...
        canvas.zoom_rect_selected.connect(self.zoom_to_rect)
    ...
    def show_unique_label_list_menu(self, point: QtCore.QPoint) -> None:
        ...
        if not color.isValid():
            return
        self._runtime_label_colors[label] = (
            color.red(),
            color.green(),
            color.blue(),
        )
    ...
    def _change_default_label_color(self) -> None:
        current_rgb = self._config["shape_color"]["uniform"]["color"]
        color = QtWidgets.QColorDialog.getColor(
            QtGui.QColor(*current_rgb),
            self,
        ...
        values = (
            ...
            (("shape_color", "uniform", "color"), rgb),
            (("shape_color", "by_label", "fallback"), rgb),
        ...
        if self._is_settings_editable:
            ...
            try:
                _config.set_overrides(config_file=self._config_file, overrides=values)
        ...
        self._config["shape_color"]["mode"] = "uniform"
        self._config["shape_color"]["uniform"]["color"] = rgb
        self._config["shape_color"]["by_label"]["fallback"] = rgb
    ...
    def add_label(self, shape: Shape) -> None:
        ...
        if (
            ...
            and class_item.checkState() == Qt.CheckState.Unchecked
        ):
            label_list_item.setCheckState(Qt.CheckState.Unchecked)
    ...
    def _refresh_class_color(self, *, label: str) -> None:
        ...
        if class_item is not None:
            self._docks.unique_label_list.set_item_label(class_item, label, rgb)
        for label_item in self._docks.label_list:
            shape = label_item.shape()
            if shape is None or shape.label != label:
                continue
            label_item.set_label(text=format_shape_label(shape), color=rgb)
        self._canvas_widgets.canvas.update()
    ...
    def _on_unique_label_item_changed(self, item: QtWidgets.QListWidgetItem) -> None:
        ...
        new_state = Qt.CheckState.Checked if is_visible else Qt.CheckState.Unchecked
        changed = False
        with QtCore.QSignalBlocker(self._docks.label_list._model):
            for label_item in self._docks.label_list:
                shape = label_item.shape()
                if shape is None or shape.label != label:
                    continue
                label_item.setCheckState(new_state)
                if shape.visible == is_visible:
                    continue
                self._canvas_widgets.canvas.set_shape_visible(
                    shape=shape, value=is_visible
                )
                changed = True
        if changed:
            self._canvas_widgets.canvas.backup_shapes()
    ...
    def _on_new_shape(self) -> None:
        ...
        if is_point_mode:
            cached = self._point_mode_label_state
            if text and cached and cached[0] == text:
                _, cached_flags, group_id, description = cached
                flags = dict(cached_flags)
            elif not text and cached:
                text, cached_flags, group_id, description = cached
                flags = dict(cached_flags)

        should_prompt = self._config["display_label_popup"] or not text
        if is_point_mode and text:
            should_prompt = False
        if should_prompt:
            previous_text = self._label_dialog.edit.text()
            text, popup_flags, group_id, popup_description = self._label_dialog.popup(
                text
            )
            flags = popup_flags or {}
            description = popup_description or ""
            if not text:
    ...
    def toggle_zoom_rect(self, checked: bool | None = None) -> None:
        canvas = self._canvas_widgets.canvas
        if checked is None:
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        ...
        if (
            ...
            and self._canvas_widgets.canvas.hovered_shape is not None
            and self._canvas_widgets.canvas.hovered_shape.shape_type == "point"
        ):
    ...
    def zoom_to_rect(self, rect: QtCore.QRectF) -> None:
        canvas = self._canvas_widgets.canvas
        if canvas.pixmap.isNull():
            return

        p1 = canvas._transform_point_widget_to_image(rect.topLeft())
        p2 = canvas._transform_point_widget_to_image(rect.bottomRight())
        image_rect = QtCore.QRectF(p1, p2).normalized()
        if image_rect.width() < 1 or image_rect.height() < 1:
            return

        central = self.centralWidget()
        assert isinstance(central, QtWidgets.QAbstractScrollArea)
        viewport = central.viewport()
        target_scale = min(
            viewport.width() / image_rect.width(),
            viewport.height() / image_rect.height(),
        )
        self._zoom_mode = _ZoomMode.MANUAL_ZOOM
        self._set_zoom(value=max(1.0, target_scale * 100.0))

        image_center = image_rect.center()
        origin = canvas._compute_image_origin_offset()
        canvas_x = (image_center.x() + origin.x()) * canvas.scale
        canvas_y = (image_center.y() + origin.y()) * canvas.scale
        self.set_scroll_value(
            Qt.Orientation.Horizontal, canvas_x - viewport.width() / 2
        )
        self.set_scroll_value(Qt.Orientation.Vertical, canvas_y - viewport.height() / 2)
        self._zoom_rect_button.setChecked(False)
    ...
    def remove_selected_point(self) -> None:
        ...
        if not canvas.remove_selected_point():
            return
        if canvas.hovered_shape and len(canvas.hovered_shape.points) == 0:
            canvas.delete_shape(canvas.hovered_shape)
            self.remove_labels([canvas.hovered_shape])
            if self.has_no_shapes():
                for action in self._actions.on_shapes_present:
                    action.setEnabled(False)
        self.mark_dirty()
    ...
    def delete_selected_shapes(self) -> None:
        canvas = self._canvas_widgets.canvas
        selected_shapes = canvas.selected_shapes
        if not selected_shapes:
            return
        if any(shape.shape_type != "point" for shape in selected_shapes):
            msg = self.tr(
