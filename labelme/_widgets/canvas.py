...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:  # noqa: ANN401
        ...
        self._polygon_hold_timer = QtCore.QTimer(self)
        self._polygon_hold_timer.setInterval(POLYGON_HOLD_ADD_INTERVAL_MS)
        self._polygon_hold_timer.timeout.connect(self._append_held_polygon_point)
    ...
    @property
    def zoom_rect_enabled(self) -> bool:
        return self._zoom_rect_enabled

    def set_zoom_rect_mode(self, enabled: bool) -> None:
        self._zoom_rect_enabled = enabled
    ...
    def _render_context(self, shape: Shape, *, highlighted: bool) -> ShapeRenderContext:
        ...
        return ShapeRenderContext(
            ...
            selected=selected,
            fill=self._fill_editing and (selected or shape is self.hovered_shape),
    ...
    def is_drawing(self) -> bool:
        return self._current is not None or self.is_drawing_skeleton

    @property
    def is_drawing_skeleton(self) -> bool:
        return self._skeleton_drawing_mode is not None
    ...
    def set_skeleton_drawing_mode(self, mode: _SkeletonDrawingMode) -> None:
        if not self.is_drawing_skeleton:
            return
        self._skeleton_drawing_mode = mode
        self._skeleton_edge_start = None
        self.update()
        self._update_status()

    def add_skeleton_node(self, *, name: str, point: QPointF) -> None:
        if self._skeleton_drawing_mode != "nodes":
            raise RuntimeError("skeleton nodes can only be added in node mode")
        name = name.strip()
        if not name:
            raise ValueError("skeleton node name must not be empty")
        if name in self._skeleton_node_names:
    ...
    def skeleton_drawing(self) -> SkeletonDrawingResult:
        if not self.is_drawing_skeleton:
            raise RuntimeError("no skeleton drawing is active")
        return SkeletonDrawingResult(
            names=tuple(self._skeleton_node_names),
            points=tuple(QPointF(point) for point in self._skeleton_node_points),
            edges=tuple(self._skeleton_edges),
        )

    def take_skeleton_drawing(self) -> SkeletonDrawingResult:
        result = self.skeleton_drawing()
        self._clear_skeleton_drawing()
        return result

    def cancel_skeleton_drawing(self) -> None:
        if not self.is_drawing_skeleton:
            return
        self._clear_skeleton_drawing()
        self.skeleton_drawing_cancelled.emit()

    def _clear_skeleton_drawing(self) -> None:
        self._skeleton_drawing_mode = None
        self._skeleton_node_names = []
        self._skeleton_node_points = []
        self._skeleton_edges = []
        self._skeleton_edge_start = None
    ...
    def _update_status(self, extra_messages: list[str] | None = None) -> None:
        ...
        if self._skeleton_drawing_mode == "nodes":
            ...
            return
        if self._skeleton_drawing_mode == "edges":
            messages.append(self.tr("Connecting skeleton nodes"))
            if self._skeleton_edge_start is None:
                messages.append(self.tr("Click the first node of a bone"))
            else:
    ...
    def _dispatch_pointer_move(self, pos: QPointF, event: QtGui.QMouseEvent) -> None:
        if self._zoom_rect_start is not None:
            self._zoom_rect_end = event.position()
            self.update()
            return
    ...
    def _dispatch_pointer_press(self, pos: QPointF, event: QtGui.QMouseEvent) -> None:
        ...
        if button == Qt.MouseButton.LeftButton:
            ...
            if self._zoom_rect_enabled:
                self._zoom_rect_start = event.position()
                self._zoom_rect_end = event.position()
                self._apply_cursor(CursorRole.DRAW)
                self.update()
                return
            ...
            if (
                ...
                self._polygon_hold_timer.start()
            return
    ...
    def _finish_zoom_rect(self) -> None:
        start = self._zoom_rect_start
        end = self._zoom_rect_end
        self.set_zoom_rect_mode(False)
        self._release_cursor()
    ...
    def _append_held_polygon_point(self) -> None:
        current = self._current
        if (
            not self._left_button_held
            or self.mode != _CanvasMode.CREATE
            or self.create_mode != "polygon"
            or current is None
            or len(self._line.points) < 2
        ):
            self._polygon_hold_timer.stop()
            return
        ...
        if (
            np.linalg.norm(
                np.array([point.x() - previous.x(), point.y() - previous.y()])
            )
            < min_step
        ):
            return
        current = current.add_point(point, autoclose=True)
        self._current = current
        self._line = dataclasses.replace(
            self._line, points=(current.points[-1],) + self._line.points[1:]
        )
    ...
    def paintEvent(self, a0: QtGui.QPaintEvent) -> None:
        if self.pixmap.isNull():
            ...
            return
        self._render_canvas()
        self._draw_zoom_rect_overlay()
        if self._current is not None:
            self._clear_highlight_state()

    def _draw_zoom_rect_overlay(self) -> None:
        if self._zoom_rect_start is None or self._zoom_rect_end is None:
            return
        ...
        try:
            ...
            painter.setPen(
                QtGui.QPen(
                    QtGui.QColor(0, 120, 215, 200),
                    2,
                    Qt.PenStyle.DashLine,
                )
            )
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.drawRect(QRectF(self._zoom_rect_start, self._zoom_rect_end))
        finally:
            painter.end()
    ...
    def _render_layers(self) -> tuple[Callable[[QtGui.QPainter], None], ...]:
        ...
        return (
            ...
            self._draw_center_dots_layer,
            self._draw_skeleton_drawing_layer,
            self._draw_active_shape_layer,
            self._draw_drag_copy_layer,
            self._draw_preview_overlay_layer,
        )

    def _draw_skeleton_drawing_layer(self, painter: QtGui.QPainter) -> None:
        if not self.is_drawing_skeleton:
            return
        painter.save()
        try:
            line_color = self._draft_palette.line
            ...
            if self._skeleton_edge_start is not None:
                ...
                painter.drawLine(
                    self._skeleton_node_points[self._skeleton_edge_start] * self.scale,
                    self._prev_move_point * self.scale,
                )
                painter.setPen(pen)
    ...
    def _select_skeleton_edge_endpoint(self, *, pos: QPointF) -> None:
        ...
        if self._skeleton_edge_start is None:
            ...
            start = self._skeleton_edge_start
            self._skeleton_edge_start = None
            if start == index:
                self.update()
                return
    ...
    def _draw_center_dots_layer(self, painter: QtGui.QPainter) -> None:
        ...
        try:
            ...
            painter.setBrush(QtGui.QColor(210, 210, 0))
            radius = 2.5
            for shape in self.shapes:
                if not shape.visible or shape.shape_type not in (
                    "rectangle",
                    "oriented_rectangle",
                    "polygon",
                ):
                ...
                if len(points) == 0:
                    continue
                center = points.mean(axis=0) * self.scale
    ...
    def _reset_interaction_state(self) -> None:
        was_drawing_skeleton = self.is_drawing_skeleton
        self._skeleton_drawing_mode = None
        self._skeleton_node_names = []
        self._skeleton_node_points = []
        self._skeleton_edges = []
